...
// Store simple atts for load more
$pagination_loadmore = false; // function to be released in future update
if ( isset( $atts['pagination_loadmore'] ) && 'true' == $atts['pagination_loadmore'] ) {
	$og_atts = $atts;
}
...
if ( $wpex_query->have_posts() ) :
	...
	if ( 'masonry' == $grid_style && 'true' != $pagination_loadmore && 'true' != $filter ) {
	...
	$output .= '<div class="'. $wrap_classes .'"'. vcex_get_unique_id( $unique_id ) .'>';
									...
									$output .= '<div class="'. $media_classes .'">';
										...
										if ( $thumb_link == 'post'
											...
											$output .= '<a '. $media_link_attrs .'>';
												...
												$output .= wpex_get_entry_media_after( 'vcex_post_type_grid' );
		...
		if ( 'true' == $pagination
			...
			if ( 'true' == $pagination_loadmore ) {
				$output .= vcex_loadmore_button( 'vcex_post_type_grid', $og_atts, $wpex_query );
			} else {
