...
$output = '<div ' . wpex_parse_attrs( $wrap_attrs ) . '>';
	...
	if ( 'true' == $overlay ) {
		...
		$overlay_style = $overlay_color ? ' ' . vcex_inline_style( array(
		...
		$output .= '<div class="vcex-ib-content clr"' . $inner_style . '>';
			...
			if ( $heading ) {
				...
				if ( $rfont_size = vcex_get_responsive_font_size_data( $heading_font_size ) ) {
					...
					$rdata                     = array( 'font-size' => $rfont_size );
					$attrs['data']             = "data-wpex-rcss='" . json_encode( $rdata ) . "'";
					$attrs['data-wpex-uniqid'] = '.' . $uniqid;
					$attrs['class']           .= ' wpex-rcss '. $uniqid;
				}
			...
			if ( $has_button ) {
				...
				if ( $button_custom_hover_color ) {
					$attrs['data-hover-color'] = $button_custom_hover_color;
					$attrs['class'] .= ' wpex-data-hover';
				}

				if ( $button_custom_hover_background ) {
					$attrs['data-hover-background'] = $button_custom_hover_background;
					$attrs['class'] .= ' wpex-data-hover';
				}
