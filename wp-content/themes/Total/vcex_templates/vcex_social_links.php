...
$output .= '<div ' . wpex_parse_attrs(  $wrap_attrs ) . '>';
	...
	foreach ( $loop as $key => $val ) {
		...
		// Link output
		if ( $url ) {
			$output .= '<a href="'. esc_url( do_shortcode( $url ) ) .'" class="'. implode( ' ', $a_classes ) .' wpex-'. $profile_class .'"'. $attributes .'><span class="'. $social_profiles[$key]['icon_class'] .'" aria-hidden="true"></span><span class="screen-reader-text">' . $key . '</span></a>';
		}
